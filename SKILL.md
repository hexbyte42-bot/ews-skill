...
- Instruct users to edit `.env` manually on their machine; never request, collect, or echo passwords, tokens, `GRAPH_CLIENT_ID`, client secrets, or other sensitive values in chat.
...
- Never ask users to paste secret values (including `GRAPH_CLIENT_ID`) into chat; use placeholders and tell them to fill sensitive values directly in `.env`.
